...
$stmt = $pdo->prepare("SELECT * FROM news WHERE type = 'Sports News'");
$stmt->execute();
$sportsNews = $stmt->fetchAll(PDO::FETCH_ASSOC);
...
<body style="background: #fff0f7;">
    ...
    <div style="padding-left: 30px; padding-right: 31px;">
        <h2 class="text-center" style="margin-bottom: 25px;">Sports News</h2> 

        <?php
        $description = $sportsNews[0]['description'];
        ...
        <div class="card border-0 shadow-sm">
            <div class="row g-0">
                <div class="col-md-4">
                    <a href="newsview.php?id=<?= $sportsNews[0]['id'] ?>">
                        <img src="<?= $sportsNews[0]['img_src'] ?>" class="card-img-top1" alt="...">
                    </a>
                </div>
                <div class="col-md-8">
                    <div class="card-body">
                        <h4 class="card-title">
                            <a style="text-decoration: none; color: black;" href="newsview.php?id=<?= $sportsNews[0]['id'] ?>">
                                <?= $sportsNews[0]['title'] ?>
                            </a>
                        </h4>
                        <p class="card-text" id="bigNewsDescription"><?= $firstSentence ?></p>
                        <form id="bigCardForm" method="POST" action="newsview.php">
                            <input type="hidden" name="id" id="bigCardId" value="<?= htmlspecialchars($sportsNews[0]['id']) ?>">
        ...
        <div class="row row-cols-1 row-cols-md-4 g-5" style="padding-top: 12px;">
            ...
            for ($i = 0; $i < $nr - $index; $i++) {
                $description = $sportsNews[$index + $i]['description'];
                $firstSentenceEnd = strpos($description, '.');
                $firstSentence = ($firstSentenceEnd !== false) ? substr($description, 0, $firstSentenceEnd + 1) : $description;
            ?>
                <div class="col">
                    <div class="card"> 
                        <img src="<?= $sportsNews[$index + $i]['img_src'] ?>" class="card-img-top1" alt="...">
                        <div class="card-body">
                            <h5 class="card-title"><?= $sportsNews[$index + $i]['title'] ?></h5>
                            <p class="card-text" id="bigNewsDescription"><?= $firstSentence ?></p>
                            <form method="POST" action="newsview.php">
                            <input type="hidden" name="id" value="<?= htmlspecialchars($sportsNews[$index + $i]['id']) ?>">
